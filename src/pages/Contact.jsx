  ...
  const offices = [
    {
      ...
      phone: "+65 6789 0123",
      email: "sg@zephyrglobal.com",
      hours: "09:00 - 18:00 (SGT)"
    },
    {
      city: "Rotterdam Hub",
      address: "Willemswerf Building, Boompjes 40, 3011 XB Rotterdam, Netherlands",
      phone: "+31 10 567 8900",
      email: "rotterdam@zephyrglobal.com",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-3xl mx-auto space-y-4">
            ...
            <p className="text-slate-350 text-base leading-relaxed">
              Connect with our logistics coordinators, compliance auditors, and container booking agents to coordinate your next movement.
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-5 space-y-8">
              <div className="space-y-4">
                <h2 className="text-xl font-bold text-white">Our Regional Headquarters</h2>
                <p className="text-slate-400 text-sm leading-relaxed">
                  We maintain strategic control offices located at critical maritime junctions and shipping hubs to expedite port releases.
                </p>
              </div>
              ...
              <div className="space-y-4">
                {offices.map((office, idx) => (
                  <div key={idx} className="glass-panel p-5 rounded-2xl border-slate-800/40 relative overflow-hidden group">
                    ...
                    <div className="space-y-2 text-xs text-slate-400">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <a href={`mailto:${office.email}`} className="hover:text-primary-400 transition-colors">{office.email}</a>
                      </div>
            ...
            <div className="lg:col-span-7">
              <div className="glass-panel p-8 rounded-3xl border-slate-800/40 shadow-2xl relative min-h-[500px]">
                {submitted ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 bg-navy-950/95 backdrop-blur-md rounded-3xl space-y-6 animate-fade-in z-25">
                    ...
                    <div className="space-y-2 max-w-md">
                      <h3 className="text-2xl font-bold text-white">Freight Request Logged</h3>
                      <p className="text-slate-400 text-sm leading-relaxed">
                        Thank you, <strong>{formData.name}</strong>. Your routing inquiry has been assigned to our hub logistics coordinator.
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-navy-900 border border-slate-850 w-full max-w-sm text-left text-xs space-y-2 text-slate-450">
                      <div className="flex justify-between"><span className="text-slate-500 font-bold">Quote Ref:</span> <span className="font-mono text-white font-bold">{quoteRef}</span></div>
                      <div className="flex justify-between"><span className="text-slate-500 font-bold">Inquiry Mode:</span> <span className="text-white uppercase font-bold">{formData.mode} Freight</span></div>
                      <div className="flex justify-between"><span className="text-slate-500 font-bold">Est. Response:</span> <span className="text-primary-400 font-semibold flex items-center gap-1"><FileCheck className="w-3.5 h-3.5" /> Under 2 Hours</span></div>
                    </div>

                    <button
                      onClick={() => {
                        setSubmitted(false);
                        setFormData({ name: '', company: '', email: '', phone: '', mode: 'ocean', message: '' });
                      }}
                ...
                <div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-bold text-white">Route Coordination Request</h3>
                    <p className="text-xs text-slate-500">Provide shipping coordinates to receive a rate proposal.</p>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    {/* Mode selector */}
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Primary Logistic Mode</label>
                      <select
                        name="mode"
                        value={formData.mode}
                        onChange={handleInputChange}
                      ...
                      >
                        <option value="ocean">Ocean Container (FCL / LCL)</option>
                        <option value="air">Priority Air Cargo</option>
                        <option value="road">Overland Rail & Road</option>
                        <option value="customs">Customs Clearance & HS Tariff Consult</option>
                      </select>
                    </div>

                    {/* Details Message */}
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Routing Details & Cargo Weight *</label>
                      <textarea
                        name="message"
      ...
      <section className="py-24 relative overflow-hidden bg-navy-900/35 border-t border-slate-900">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="text-center max-w-2xl mx-auto space-y-3 mb-16">
            ...
            <h2 className="text-xs font-bold text-primary-500 uppercase tracking-widest">Inquiries FAQ</h2>
            <h3 className="text-3xl font-extrabold text-white">Customs & Shipping Assistance</h3>
          </div>
          ...
          <div className="space-y-4">
            {faqs.map((faq, idx) => {
              ...
              return (
                <div key={idx} className="glass-panel rounded-2xl border-slate-800/40 overflow-hidden transition-all duration-300">
                  <button
                    onClick={() => toggleFaq(idx)}
                    className="w-full flex items-center justify-between p-6 text-left hover:bg-navy-900/40 transition-colors"
                  >
                    <span className="font-semibold text-slate-100 pr-4 text-sm sm:text-base">{faq.q}</span>
